...
const jeefo = (() => {
  ...
  const resolve_filepath = filepath => {
    const paths = [
      filepath,
      `${filepath}.js`,
      `${filepath}.json`,
    ...
    return paths.find(fp => modules[fp]);
  };
  ...
  class Jeefo {
    ...
    async require(path, from, current_dir = null) {
      ...
      this.pendings[path] = new Promise(async (resolve, reject) => {
        ...
        try {
          await this.load_script(url.href);
...
if (typeof window !== "undefined" && !window.jeefo) {
  window.jeefo = jeefo;
}