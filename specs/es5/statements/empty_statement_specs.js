...
describe("EmptyStatement", () => {
	...
	it('Type should be "EmptyStatement"', function () {
		expect(stmt.type).toBe("EmptyStatement");
	});
	...
	it("Should be has end object", function () {
		...
		expect(stmt.end.index).toBe(1);
		expect(stmt.end.column).toBe(2);
		expect(stmt.end.virtual_column).toBe(2);
	});
});
